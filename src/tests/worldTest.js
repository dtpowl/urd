...
function run(window, document) {
  ...
  action.onSuccess((world) => { actionCallbackCalled = true; })
  if (!actionCallbackCalled) { throw "failure"; }
  ...
  if (!w0.check('friends', 'nyar', 'daniel')) { throw "failure"; }
  if (!w0.check('friends', 'nyar', 'zoe')) { throw "failure" }

  if (w0.check('locatedIn', 'nyar', 'den')) { throw 'failure'; }

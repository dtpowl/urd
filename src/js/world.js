...
export class World {
  constructor({
    ...
    this._stateObservers = stateObservers;
  ...
  _applyEvents(triggeredObservers=new Set()) {
    ...
    const nextEvents = []; // queue of events generated by observers
    this._events.forEach((event) => {
      this._modelObservers.map((o) => o.prepare(this));

      this._model.assert(event);

      const observerEffects = this._modelObservers.map((o) => {
        const effect = o.consider(this);
        if (effect && triggeredObservers.has(o)) {
          debugger
          throw 'Observer triggered twice!';
        } else if (effect) {
          triggeredObservers.add(o);
          nextEvents.push(...effect.events);
        }
      });
    });

    this._stateObservers.forEach((o) => {
      const effect = o.consider(this);
      if (effect && !triggeredObservers.has(o)) {
        triggeredObservers.add(o);
        nextEvents.push(...effect.events);
  ...
  next(events, { lastAction }={}) {
    ...
    return new World({
      ...
      modelObservers: this._modelObservers,
      stateObservers: this._stateObservers,
